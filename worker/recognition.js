...
export async function analyzeFloorPlan(image,width,height,key,model='gpt-6-astra',fetcher=fetch) {
  const response=await fetcher('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},signal:AbortSignal.timeout(150000),body:JSON.stringify({
...
export async function recognitionApi(request,env) {
  ...
  try{return json(await analyzeFloorPlan(image,width,height,env.OPENAI_API_KEY,env.FLOOR_PLAN_MODEL||'gpt-6-astra'));}
  catch(e){return json({error:e instanceof Error&&/^(Image analysis|Analysis did|This image|The scan|The detected|A printed|Invalid analysis)/.test(e.message)?e.message:'The plan could not be analyzed. Please try again.'},502);}
}
