...
export async function shares(request,env){
 ...
 if(request.method==='GET'&&id){const row=await env.DB.prepare('SELECT document FROM shared_plans WHERE id = ?').bind(id).first();return row?json({plan:await decodeStoredPlan(row.document)}):json({error:'This shared link was not found.'},404);}
 ...
 const {studioDrafts,...snapshot}=plan;
 let document;try{document=await encodeStoredPlan(snapshot);}catch(error){if(error instanceof RangeError)return json({error:error.message},413);throw error;}
