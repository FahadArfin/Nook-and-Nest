...
The Google controls offer Lighter (16 px, 256 MiB), Balanced (8 px, 512 MiB), Smooth rotation (8 px, 1024 MiB), and Sharper nearby (4 px, 1024 MiB). Devices reporting at least 8 GiB RAM start with Smooth rotation; those reporting 4 GiB or less start with Lighter. Devices between those values, or browsers without a memory hint, use Balanced. Smooth rotation spends extra memory on retaining visited views rather than increasing geometry detail. These are decoded tile memory estimates, not total browser or GPU memory guarantees. An in-flight tile can briefly exceed the target. No settings can guarantee maximum source detail for every possible view at once.
