...
3. **Finding pieces.** Room collections cross catalog categories, including appropriate lighting. Browse, Saved and Recent remain separate views. Recent contains the last 12 confirmed pieces. Selected filters stay highlighted; Clear all sits beside Filters. Cards show the thumbnail, name and dimensions, with tags available inside Filters. The library uses a single heading.
