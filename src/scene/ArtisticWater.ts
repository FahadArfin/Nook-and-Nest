...
void main(){
 vec2 p=vWorld;float h=swell(p);vec3 normal=normalize(vec3((h-swell(p+vec2(.025,0.0)))/.025,1.0,(h-swell(p+vec2(0.0,.025)))/.025));
 vec3 view=normalize(eye-vec3(p.x,-.2,p.y));float facing=max(dot(normal,view),0.0);float fresnel=.035+.55*pow(1.0-facing,5.0);
...
export function artisticWater(scene:Scene){const material=new ShaderMaterial('artistic-river',scene,'artisticWater',{attributes:['position','uv','waterDepth'],uniforms:['worldViewProjection','time','night','eye'],needAlphaBlending:true});material.backFaceCulling=false;material.setFloat('time',0);material.setFloat('night',0);const observer=scene.onBeforeRenderObservable.add(()=>{if(scene.activeCamera)material.setVector3('eye',scene.activeCamera.globalPosition)});material.onDisposeObservable.add(()=>scene.onBeforeRenderObservable.remove(observer));return material;}
