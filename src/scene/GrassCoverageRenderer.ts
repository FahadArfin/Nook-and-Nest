...
import {meadowSample} from '../meadowScatter';
...
export class GrassCoverageRenderer{
 ...
 for(let ring=0;ring<=20&&items.length<12000;ring++)for(let x=cx-ring;x<=cx+ring&&items.length<12000;x++)for(let z=cz-ring;z<=cz+ring&&items.length<12000;z++){if(Math.max(Math.abs(x-cx),Math.abs(z-cz))!==ring)continue;const density=coverage[x+':'+z];if(!density)continue;const count=Math.min(25,4+density*2);for(let i=0;i<count&&items.length<12000;i++){const scatter=meadowSample(x,z,i),px=scatter.x,pz=scatter.z;if(rects.some(r=>px*1000>=r.x&&px*1000<=r.x+r.width&&pz*1000>=r.z&&pz*1000<=r.z+r.depth))continue;const ground=sample(px,pz);if(ground.water||mask(px,pz,.1))continue;items.push({id:'coverage:'+x+':'+z+':'+i,catalogId:c.id,floorId:floor.id,x:px*1000,z:pz*1000,widthMm:Math.round(c.widthMm*scatter.widthScale),depthMm:Math.round(c.depthMm*scatter.widthScale),heightMm:Math.round(c.heightMm*scatter.heightScale),rotation:scatter.rotation,variant:'sage',elevationMm:ground.height*1000-floor.elevationMm-50})}}
