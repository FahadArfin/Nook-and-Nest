...
import { findFloorFinish, findWallFinish, type SurfaceFinish } from "../surfaces";
...
export class SceneController {
  private engine: Engine; private scene: Scene; private camera: ArcRotateCamera; private root: TransformNode; private callbacks: Callbacks; private tool: Tool = "select"; private dragging?: string; private draggingDraft = false; private tileDragStart?: TileCell; private tileDragCurrent?: TileCell; private tileDraftRoot?: TransformNode; private tileDraftCells: TileCell[]=[]; private tileDraftPresent=true; private tileDraftAnchor?: Vector3; private surfaceMaterials=new Map<string,StandardMaterial>(); private activePlan?: PlanDocumentV1; private activeFloorId = ""; private selectedId?: string; private activeDraft?: FurniturePlacement; private previewNode?: TransformNode; private selectedNode?: TransformNode; private draftPosition?: { x: number; z: number }; private shadow: ShadowGenerator; private furnitureFactory: FurnitureFactory; private furnitureModels: FurnitureModelLibrary;
  ...
  private buildWall(id: string, ax:number,az:number,bx:number,bz:number,y:number,ghost:boolean,finish:SurfaceFinish,opening?:"door"|"window") { const length=Math.hypot(bx-ax,bz-az); const horizontal=Math.abs(az-bz)<.001; const facesCamera=(horizontal&&bx>ax)||(!horizontal&&bz>az); const wall=MeshBuilder.CreateBox(`wall:${id}`,{width:length,height:1.65,depth:.1},this.scene); wall.parent=this.root; wall.position=new Vector3((ax+bx)/2,y+.84,(az+bz)/2); wall.rotation.y=-Math.atan2(bz-az,bx-ax); wall.material=this.surfaceMaterial(`wall-mat-${id}`,finish,ghost?.18:facesCamera?.22:.94); wall.isPickable=!ghost&&(this.tool==="door"||this.tool==="window"); wall.receiveShadows=true; this.shadow.addShadowCaster(wall); if(opening){ const marker=MeshBuilder.CreateBox(`opening:${id}`,{width:opening==="door"?.68:.8,height:opening==="door"?1.25:.68,depth:.12},this.scene); marker.parent=this.root; marker.position=wall.position.clone(); marker.position.y=y+(opening==="door"?.64:1.02); marker.rotation.y=wall.rotation.y; marker.material=this.material(`opening-mat-${id}`,opening==="door"?"#8a6046":"#9ec8c3",ghost?.15:facesCamera?.25:.95); marker.isPickable=!ghost&&(this.tool==="door"||this.tool==="window"); } }
