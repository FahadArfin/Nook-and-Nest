...
export class GrassRenderer{
 ...
 private prototype(item:FurniturePlacement,ghost:boolean){
  const key=JSON.stringify([item.variant,item.materialColors,ghost]);if(this.prototypes.has(key))return this.prototypes.get(key)!;
  const def=catalog.find(c=>c.id==='grass-clump')!,node=new TransformNode('grass-source',this.scene);
 ...
 update(plan:PlanDocumentV1,floorId:string,selected?:string,draft?:string){
  ...
  const items=plan.furniture.filter(p=>p.catalogId==='grass-clump'&&p.id!==selected&&p.id!==draft&&(p.floorId===floorId||p.floorId===below||plan.camera.mode==='dollhouse'));
  ...
  const groups=new Map<string,FurniturePlacement[]>();for(const item of items){const key=JSON.stringify([item.floorId,item.floorId!==floorId,Math.floor(item.x/6000),Math.floor(item.z/6000),item.variant,item.materialColors]);const list=groups.get(key)??[];list.push(item);groups.set(key,list)}
