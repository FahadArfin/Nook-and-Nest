...
export class TerrainScene{
  private simulation?:ShallowWater;private waterPositions:Float32Array=new Float32Array();private waterDepth:Float32Array=new Float32Array();private bounds?:{minX:number;minZ:number;dx:number;dz:number};private projectId='';private architecture='';private accumulator=0;
  ...
  update(plan:PlanDocumentV1){
    ...
    const n=160,positions:number[]=[],indices:number[]=[],uvs:number[]=[],seeds:number[]=[],beds:number[]=[],blocked:number[]=[],foundations=plan.floors.flatMap(f=>floorRects(f,plan.gridSizeMm));
    for(let z=0;z<=n;z++)for(let x=0;x<=n;x++){const px=minX+x*(maxX-minX)/n,pz=minZ+z*(maxZ-minZ)/n;const ground=sample(px,pz);positions.push(px,ground.height,pz);beds.push(ground.height);blocked.push(foundations.some(r=>px>=r.x/1000-.25&&px<=(r.x+r.width)/1000+.25&&pz>=r.z/1000-.25&&pz<=(r.z+r.depth)/1000+.25)?1:0);seeds.push(ground.water?1:0);uvs.push(x/n,z/n);}
    ...
    if(strokes.some(s=>s.kind==='river')){
      ...
      this.simulation.setTerrain(beds,seeds,blocked);
      if(!this.water||this.waterPositions.length!==positions.length){this.water?.dispose();this.water=make('river-surface',positions,indices,uvs);this.water.material=this.river;this.water.setVerticesData('waterDepth',new Float32Array(beds.length),true,1);this.water.setVerticesData('waterFlow',new Float32Array(beds.length*2),true,2);}
      this.waterPositions=new Float32Array(positions);this.waterDepth=new Float32Array(beds.length);this.syncWater();
  ...
  private syncWater(){
    ...
    for(let i=0;i<sim.depth.length;i++){this.waterDepth[i]=sim.depth[i];this.waterPositions[i*3+1]=sim.bed[i]+sim.depth[i]+.002;}
    ...
    mesh.updateVerticesData('waterFlow',sim.flow);
  }
