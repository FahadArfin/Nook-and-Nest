...
export class TerrainScene{
  ...
  update(plan:PlanDocumentV1){
    ...
    const architecture=JSON.stringify([plan.gridSizeMm,plan.floors.map(f=>[f.cells,f.cellRects])]);if(this.projectId!==plan.id){this.simulation=undefined;this.projectId=plan.id;this.architecture=architecture;}this.architecture=architecture;const strokes=plan.environment?.terrain;if(!strokes?.length){this.ground?.dispose();this.ground=undefined;this.gridKey='';this.target=[];this.water?.dispose();this.water=undefined;this.simulation=undefined;return;}
    ...
    const field=this.field.sample(plan,minX,minZ,(maxX-minX)/n,(maxZ-minZ)/n,n);
    for(let z=0;z<=n;z++)for(let x=0;x<=n;x++){const px=minX+x*(maxX-minX)/n,pz=minZ+z*(maxZ-minZ)/n;const ground=field[z*(n+1)+x];positions.push(px,ground.height,pz);beds.push(ground.height);blocked.push(foundations.some(r=>px>=r.x/1000-.25&&px<=(r.x+r.width)/1000+.25&&pz>=r.z/1000-.25&&pz<=(r.z+r.depth)/1000+.25)?1:0);seeds.push(ground.water?1:0);levels.push(ground.waterLevel??-.2);uvs.push(x/n,z/n);}
    ...
    if(strokes.some(s=>s.kind==='river')){
      ...
      if(!this.water||this.waterPositions.length!==renderCount*3){
        ...
        this.water=make('river-surface',rp,[],ru);this.wetCells=new Uint8Array();this.water.material=this.river;
  ...
  isWet(x:number,z:number){const sim=this.simulation,b=this.bounds;if(!sim||!b)return false;const ix=Math.round((x-b.minX)/b.dx),iz=Math.round((z-b.minZ)/b.dz);return ix>=0&&iz>=0&&ix<sim.size&&iz<sim.size&&sim.depth[iz*sim.size+ix]>.025;}
  private advanceWater(dt:number){
    ...
    while(this.accumulator>=1/30){changed=!!this.simulation.step()||changed;this.accumulator-=1/30;}
  ...
  private syncWater(){
    ...
    const cells=(renderSize-1)**2;if(this.wetCells.length!==cells)this.wetCells=new Uint8Array(cells);let topologyChanged=false;
    for(let z=0;z<renderSize-1;z++)for(let x=0;x<renderSize-1;x++){const a=z*renderSize+x,i=z*(renderSize-1)+x,wet=Math.max(this.waterDepth[a],this.waterDepth[a+1],this.waterDepth[a+renderSize],this.waterDepth[a+renderSize+1])>.0001?1:0;if(this.wetCells[i]!==wet){this.wetCells[i]=wet;topologyChanged=true}}
    if(topologyChanged){const indices:number[]=[];for(let i=0;i<this.wetCells.length;i++)if(this.wetCells[i]){const a=Math.floor(i/(renderSize-1))*renderSize+i%(renderSize-1),b=a+1,c=a+renderSize,d=c+1;indices.push(a,c,b,b,c,d)}mesh.setIndices(indices);}
