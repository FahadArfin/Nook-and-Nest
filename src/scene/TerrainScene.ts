...
export class TerrainScene{
  private positions:number[]=[];private target:number[]=[];private indices:number[]=[];private gridKey='';private ground?:Mesh;private water?:Mesh;private key='';private soil:StandardMaterial;private river:StandardMaterial;private texture:DynamicTexture;
  constructor(private scene:Scene){
    ...
    this.river=new StandardMaterial('flowing-river',scene);this.river.diffuseColor=Color3.FromHexString('#80b9bb');this.river.specularColor=new Color3(.2,.3,.3);this.river.alpha=.84;this.river.backFaceCulling=false;
    ...
    scene.onBeforeRenderObservable.add(()=>{
      if(this.ground&&this.target.length){let changed=false;const a=window.matchMedia?.('(prefers-reduced-motion: reduce)').matches?1:1-Math.exp(-scene.getEngine().getDeltaTime()/160);for(let i=1;i<this.positions.length;i+=3){const d=this.target[i]-this.positions[i];if(Math.abs(d)>.001){this.positions[i]+=d*a;changed=true}else this.positions[i]=this.target[i];}if(changed){const normals:number[]=[];VertexData.ComputeNormals(this.positions,this.indices,normals,{useRightHandedSystem:this.scene.useRightHandedSystem});this.ground.updateVerticesData(VertexBuffer.PositionKind,this.positions,true);this.ground.updateVerticesData(VertexBuffer.NormalKind,normals);}}if(!window.matchMedia?.('(prefers-reduced-motion: reduce)').matches)this.texture.vOffset=(this.texture.vOffset+scene.getEngine().getDeltaTime()*.000012)%1;});
  }
  update(plan:PlanDocumentV1){
    ...
    if(!this.ground||this.gridKey!==gridKey){this.ground?.dispose();this.gridKey=gridKey;this.positions=positions.map((v,i)=>i%3===1?-.15:v);this.indices=indices;this.ground=make('sculpted-landscape',this.positions,indices,uvs);this.ground.material=this.soil;this.ground.receiveShadows=true;}this.target=positions;
