...
export function validatePlan(value: unknown): asserts value is PlanDocumentV1 {
  ...
  for (const f of p.floors) {
    ...
    if(f.blueprint!==undefined){obj(f.blueprint);str(f.blueprint.geometryKey,900000);arr(f.blueprint.rooms,100);unique(f.blueprint.rooms);for(const r of f.blueprint.rooms){str(r.name,100);if(!['Living','Bedroom','Dining','Office','Kitchen','Bathroom','Laundry','Hall','Outdoor','Closet'].includes(r.kind)||typeof r.enclosed!=='boolean')fail();num(r.x,-100000,100000);num(r.z,-100000,100000);num(r.width,10,60000);num(r.depth,10,60000);}}
