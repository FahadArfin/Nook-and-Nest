...
import { Armchair, Bed, Books, CaretLeft, CaretRight, CookingPot, Door, Flower, FrameCorners, GridFour, Lamp, Monitor, Plant, Shower, SquaresFour, Stairs, Table, Tree, Bathtub, Toilet, Fish, Flame, Clock, PawPrint, Baby, ForkKnife, Lightbulb, WashingMachine, type Icon } from '@phosphor-icons/react';
...
export function LibraryIconRail({label, values, value, onChange}: {
  ...
  const measure = () => {const el=ref.current;if(el)setEdges({start:el.scrollLeft<2,end:el.scrollLeft+el.clientWidth>=el.scrollWidth-2})};
  ...
  const scroll=(direction:number)=>ref.current?.scrollBy({left:direction*ref.current.clientWidth*.75,behavior:window.matchMedia?.('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});
  ...
  return <div className="library-icon-navigation">
    <div className="library-rail-heading"><span>{label==='Category'?'Explore categories':'Furniture types'}</span><div>
      <button aria-label={`Scroll ${label.toLowerCase()} icons left`} disabled={edges.start} onClick={()=>scroll(-1)}><CaretLeft size={13}/></button>
      <button aria-label={`Scroll ${label.toLowerCase()} icons right`} disabled={edges.end} onClick={()=>scroll(1)}><CaretRight size={13}/></button>
    </div></div>
