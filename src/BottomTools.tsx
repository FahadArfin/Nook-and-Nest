...
function LandscapeMenus({onPlace,onView,onBrowseLibrary}:{onPlace:(item:CatalogItem)=>void;onView:()=>void;onBrowseLibrary?:()=>void}){
 ...
 return <div className="landscape-menus">{shown&&<div className={`landscape-detail ${open?'is-open':''}`} aria-label={shown+' options'} aria-hidden={!open} inert={!open}><strong>{shown}</strong>{shown==='Terrain'?<TerrainSettings/>:shown==='Plants'?<PlantingSettings onPlace={onPlace}/>:<Surroundings onView={onView} onBrowseLibrary={onBrowseLibrary}/>}</div>}<nav aria-label="Landscape tools">{['Terrain','Plants','Surroundings'].map(label=><button key={label} aria-pressed={choice===label} onClick={()=>{usePlanner.getState().setTool('select');setChoice(choice===label?undefined:label)}}>{label}</button>)}</nav></div>
}
