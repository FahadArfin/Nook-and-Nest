...
export async function recognitionKey(ref:PlanReference,model:ScanModel,guidance='',wallView=false):Promise<string|undefined> {
  try {const bytes=new TextEncoder().encode(JSON.stringify([PIPELINE_VERSION,model,ref.width,ref.height,ref.url,...(guidance.trim()?[guidance.trim()]:[]),...(wallView?[{wallView:'wall-support-v1'}]:[])]));return Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',bytes)),b=>b.toString(16).padStart(2,'0')).join('');}catch{return undefined;}
}
