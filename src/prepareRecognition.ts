...
export async function prepareRecognition(ref:PlanReference,signal?:AbortSignal):Promise<ScanEvidence> {
  ...
  try {
    ...
    const walls=await new Promise<ScanEvidence['walls']>((resolve,reject)=>{
      ...
      worker.onmessage=e=>{finish();e.data.error?reject(new Error(e.data.error)):resolve(e.data.walls);};worker.onerror=()=>{finish();reject(new Error('Could not prepare image geometry.'));};
      signal?.addEventListener('abort',abort,{once:true});if(signal?.aborted){abort();return;}worker.postMessage({pixels,width:canvas.width,height:canvas.height,sourceWidth:ref.width,sourceHeight:ref.height},[pixels.buffer]);
    });
    ...
    return {version:PIPELINE_VERSION,walls,crops};
