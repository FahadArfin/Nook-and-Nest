...
export interface ScanRoom {name:string;kind:typeof detectedKinds[number];x:number;y:number;width:number;height:number;enclosed:boolean;note:string}
...
export const recognitionSchema=obj({
  rooms:array(obj({name:string,kind:{type:'string',enum:detectedKinds},x:number,y:number,width:number,height:number,enclosed:{type:'boolean'},note:string})),
