...
import { openPlanAreas, fixtureName, blueprintPlan, blueprintProblems, draftFromFloor, fixtureAt, isFixedPiece, roomKinds, roomGroups, type BlueprintDraft, type BlueprintRoom, type RoomKind } from './blueprint';
...
import {clearRecognitionCache,type ScanModel} from './recognitionCache';
...
export function BlueprintStudio({onClose,onCreated}:{onClose:()=>void;onCreated?:()=>void}) {
  ...
  function down(e:ReactPointerEvent<SVGSVGElement>) {
    ...
    const raw=point(e),hit=(e.target as Element).closest('[data-object]')?.getAttribute('data-object'),p=mode==='scale'||mode==='pan'||draft.fixtures.some(f=>f.id===hit)?raw:snap(raw);
    if(mode==='fixture') {if(!plan){setError('Draw a room area first.');return;}try{const item={...fixtureAt(plan,floorId,fixtureId,p.x,p.z),...(entrance&&isDoor(fixtureId)?{doorless:true}:{})};commit({...draft,fixtures:[...draft.fixtures,item]});setSelected(item.id);setMode('select');}catch(e){setError((e as Error).message);}return;}
  ...
  function move(e:ReactPointerEvent<SVGSVGElement>) {
    ...
    const p=gesture.mode==='scale'||gesture.mode==='pan'||gesture.before.fixtures.some(f=>f.id===gesture.selected)?raw:snap(raw);setGesture(g=>g?{...g,b:p}:g);
  ...
  return <dialog ref={dialogRef} className="blueprint-dialog" data-theme={(appearance?.dark??savedDark)?'dark':'light'} aria-labelledby="blueprint-title" onCancel={e=>{e.preventDefault();close();}} onKeyDown={e=>e.stopPropagation()}>
    ...
    {loading&&<div className="bp-analysis-status" role="status"><strong>Reading your floor plan…</strong><p>Finding rooms, printed dimensions and fixtures. Detailed scans can take a few minutes.</p><button onClick={()=>{analysisAbort.current?.abort();loadGeneration.current++;setLoading(false);setNotice('Analysis canceled. Your drawing is unchanged.');}}>Cancel analysis</button></div>}<div className="bp-workspace" style={pendingScale?{display:'none'}:undefined} inert={loading||!!pendingScale} aria-busy={loading}><aside className="bp-sidebar">
      {!review&&<>
        ...
        <div className="bp-button-row"><button disabled={!plan} onClick={()=>{setFixtureId('door-flush');setEntrance(false);setMode('fixture');}}>Add door</button><button disabled={!plan} onClick={()=>{setFixtureId('door-flush');setEntrance(true);setMode('fixture');}}>Add entrance (no door)</button></div><button disabled={!draft.rooms.length} onClick={()=>{commit({...draft,rooms:openPlanAreas(draft.rooms)});setNotice('Living/dining and hall areas are open-plan. Enclosed bedrooms, bathrooms and explicitly drawn walls remain. Select any remaining inside wall to remove it.');}}>Open living / hallway areas</button>
        <label>Kitchen / bath / laundry / openings<select value={fixtureId} onChange={e=>{setFixtureId(e.target.value);setEntrance(false);}}>{fixedCatalog.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><button disabled={!plan} className={mode==='fixture'?'active':''} onClick={()=>setMode('fixture')}>Place selected fixture</button><small>Drag yellow boxes to move fixtures. Use the list below for small items. Doors and entrances snap to walls.</small></section>
        ...
        {!!draft.fixtures.length&&<section><h2>Fixtures & openings</h2><div className="bp-room-list">{draft.fixtures.map(f=><button key={f.id} className={selected===f.id?'active':''} onClick={()=>{setSelected(f.id);setMode('select');}}>{fixtureName(f)}</button>)}</div></section>}
      ...
      <svg ref={svgRef} className={`bp-canvas mode-${mode}`} role="img" aria-label="Top-down floor plan drawing" viewBox={`${view.x} ${view.z} ${view.width} ${view.height}`} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={()=>{if(gesture)setDraft(gesture.before);setGesture(undefined);}}>
        ...
        {draft.fixtures.map(f=><g key={f.id} data-object={f.id} transform={`translate(${f.x},${f.z}) rotate(${f.rotation})`} className="bp-object"><rect x={-f.widthMm/2} y={-f.depthMm/2} width={f.widthMm} height={f.depthMm} rx="30" fill={f.doorless?'#fffaf0':isWallOpening(f.catalogId)?'#e3f2f6':'#f7e4b9'} strokeDasharray={f.doorless?'70 35':undefined} stroke={selected===f.id?'#b7771e':'#627b84'} strokeWidth={selected===f.id?3:1.5} vectorEffect="non-scaling-stroke"/><text y="0" textAnchor="middle" dominantBaseline="middle" fontSize={Math.min(110,f.widthMm/7)} fill="#384d4e">{fixtureName(f)}</text></g>)}
