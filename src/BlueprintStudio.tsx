...
import { blueprintPlan, blueprintProblems, draftFromFloor, fixtureAt, isFixedPiece, roomKinds, type BlueprintDraft, type BlueprintRoom, type RoomKind } from './blueprint';
...
const colors:Record<RoomKind,string>={Living:'#d8dfc5',Bedroom:'#e6d4ce',Dining:'#eadab4',Office:'#c9dddd',Kitchen:'#ead5ad',Bathroom:'#bfdae3',Laundry:'#d1cce4',Hall:'#e5e1d6',Outdoor:'#bcd2ae'};
...
export function BlueprintStudio({onClose,onCreated}:{onClose:()=>void;onCreated?:()=>void}) {
  ...
  useEffect(()=>{dialogRef.current?.showModal();return()=>{loadGeneration.current++;};},[]);
  ...
  const room=draft.rooms.find(r=>r.id===selected),fixture=draft.fixtures.find(f=>f.id===selected);
  ...
  async function load(nextFile:File,nextPage=1,nextRotation=0,clear=false) {
    ...
    try {
      ...
      setReference(result);setFile(nextFile);setPage(nextPage);setRotation(nextRotation);setCalibrated(false);setScaleLine(undefined);setMode('scale');setImageScale(10);
      if(clear){commit(emptyDraft());setSelected(undefined);}fit(clear?[]:draft.rooms,result,10);
      setNotice('Set the scale using one printed dimension, then trace the room areas. The scan is a reference; walls are not detected automatically.');
  ...
  function move(e:ReactPointerEvent<SVGSVGElement>) {
    ...
    setDraft({...gesture.before,rooms:gesture.before.rooms.map(r=>r.id===id?{...r,x:r.x+dx,z:r.z+dz}:r),fixtures:gesture.before.fixtures.map(f=>f.id===id&&moved?moved:f)});
  }
  ...
  const remove=()=>{if(!selected)return;commit({...draft,rooms:draft.rooms.filter(r=>r.id!==selected),fixtures:draft.fixtures.filter(f=>f.id!==selected),walls:draft.walls.filter(w=>w.id!==selected),omittedWalls:[...draft.omittedWalls,selected]});setSelected(undefined);};
  const updateRoom=(patch:Partial<BlueprintRoom>)=>{if(room)commit({...draft,rooms:draft.rooms.map(r=>r.id===room.id?{...r,...patch}:r)});};
  ...
  return <dialog ref={dialogRef} className="blueprint-dialog" aria-labelledby="blueprint-title" onCancel={e=>{e.preventDefault();close();}} onKeyDown={e=>e.stopPropagation()}>
    <header className="bp-header"><div><span className="eyebrow">{floorName} · unsaved draft</span><h1 id="blueprint-title">Floor plan studio</h1></div><div className="bp-steps"><span className={reference?'complete':''}>1 Reference & scale</span><span className={!review?'active':''}>2 Trace & edit</span><span className={review?'active':''}>3 Review → 3D</span></div><button onClick={close} aria-label="Close floor plan studio"><X size={22}/></button></header>
    <div className="bp-workspace"><aside className="bp-sidebar">
      {!review&&<>
        <section><h2>Start with your floor plan</h2><p>Upload a scan or draw from measurements. Files stay in this browser.</p><button className="bp-upload" disabled={loading} onClick={()=>uploadRef.current?.click()}><FileArrowUp size={22}/>{loading?'Opening floor plan…':'Upload PDF or image'}</button><input hidden ref={uploadRef} type="file" accept=".pdf,.png,.jpg,.jpeg,.webp" aria-label="Upload floor plan reference" onChange={e=>{const f=e.target.files?.[0];e.target.value='';if(!f)return;if(draft.rooms.length&&!window.confirm('Start a new tracing draft from this file? This replaces only the unconfirmed drawing; your 3D home is unchanged.'))return;void load(f,1,0,true);}}/>
        ...
        {reference&&<div className="bp-reference-controls"><strong title={reference.name}>{reference.name}</strong><div className="bp-button-row"><label>PDF page<select aria-label="PDF page" disabled={loading||draft.rooms.length>0} value={page} onChange={e=>file&&void load(file,Number(e.target.value),rotation)}>{Array.from({length:reference.pages},(_,i)=><option key={i} value={i+1}>{i+1}</option>)}</select></label><button disabled={loading||draft.rooms.length>0} onClick={()=>file&&void load(file,page,(rotation+90)%360)}><ArrowClockwise/> Rotate scan</button></div><label>Reference opacity<input type="range" min="0" max="1" step="0.05" value={opacity} onChange={e=>setOpacity(Number(e.target.value))}/></label><small>Choose the page, orientation and scale before drawing.</small></div>}
        ...
        <section><h2>Draw & label</h2><label>New room type<select value={kind} onChange={e=>setKind(e.target.value as RoomKind)}>{roomKinds.map(k=><option key={k}>{k}</option>)}</select></label><p>Drag a rectangle for each room. Combine open areas for L-shaped spaces. Edges snap together.</p><div className="bp-button-row"><button disabled={!!reference&&!calibrated} className={mode==='room'?'active':''} onClick={()=>setMode('room')}>Draw room area</button><button disabled={!draft.rooms.length} className={mode==='wall'?'active':''} onClick={()=>setMode('wall')}>Inside wall</button></div>
        <label>Kitchen / bath / laundry / openings<select value={fixtureId} onChange={e=>setFixtureId(e.target.value)}>{fixedCatalog.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><button disabled={!plan} className={mode==='fixture'?'active':''} onClick={()=>setMode('fixture')}>Place selected fixture</button><small>Place fixtures shown in your reference. Doors and windows snap to walls.</small></section>
        {room&&<section className="bp-selection"><h2>Edit room area</h2><label>Room name<input maxLength={100} value={room.name} onChange={e=>updateRoom({name:e.target.value})}/></label><label>Room type<select value={room.kind} onChange={e=>updateRoom({kind:e.target.value as RoomKind})}>{roomKinds.map(k=><option key={k}>{k}</option>)}</select></label><div className="bp-fields">{(['x','z','width','depth'] as const).map(key=><LengthField key={key} label={`${key==='x'?'Left':key==='z'?'Top':key} (${unit})`} value={room[key]} factor={factor} onChange={v=>updateRoom({[key]:v})}/>)}</div><label className="bp-check"><input type="checkbox" checked={room.enclosed} onChange={e=>updateRoom({enclosed:e.target.checked})}/> Add room dividers</label><small>Turn dividers off for open-plan rooms. Add doors to enclosed rooms.</small><button onClick={remove}><Trash/> Remove area</button></section>}
        ...
        <section><h2>Room areas · {draft.rooms.length}</h2><div className="bp-room-list">{draft.rooms.map(r=><button key={r.id} className={r.id===selected?'active':''} onClick={()=>{setSelected(r.id);setMode('select');}}><span style={{background:colors[r.kind]}}/>{r.name}<small>{(r.width/factor).toFixed(2)} × {(r.depth/factor).toFixed(2)} {unit}</small></button>)}</div></section>
      </>}
      {review&&<section className="bp-review"><span className="eyebrow">Ready for your walkthrough</span><h2>Check the empty home</h2><p>Review room sizes, wall connections, doors, windows and fixture locations.</p><dl><dt>Room areas</dt><dd>{draft.rooms.length}</dd><dt>Kitchen / bath / laundry & openings</dt><dd>{draft.fixtures.length}</dd><dt>Loose furniture</dt><dd>None</dd></dl><p>This replaces <strong>{floorName}</strong> and its contents. Other floors stay in place. Stair connections to this floor are removed. Undo restores the previous floor.</p><p>After conversion, use <strong>Auto furnish</strong> to preview an arrangement from your library. Room types guide the choices.</p>{reference&&!calibrated&&<p className="bp-error">Calibrate the reference before converting.</p>}{problems.length>0&&<div role="alert" className="bp-error"><strong>Fix these before converting:</strong><ul>{problems.slice(0,10).map((p,i)=><li key={i}>{p}</li>)}</ul></div>}<label className="bp-check"><input type="checkbox" checked={checked} onChange={e=>setChecked(e.target.checked)}/> I checked the dimensions, openings and fixtures.</label><button className="primary" disabled={!checked||!plan||stale||!!problems.length||!!reference&&!calibrated} onClick={apply}><Check/> Confirm & create 3D home</button><button onClick={()=>{setReview(false);setChecked(false);}}>Back to editing</button></section>}
      ...
      <svg ref={svgRef} className={`bp-canvas mode-${mode}`} role="img" aria-label="Top-down floor plan drawing" viewBox={`${view.x} ${view.z} ${view.width} ${view.height}`} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={()=>{if(gesture)setDraft(gesture.before);setGesture(undefined);}}>
        ...
        {draft.rooms.map(r=><g key={r.id} data-object={r.id} className="bp-object"><rect x={r.x} y={r.z} width={Math.max(0,r.width)} height={Math.max(0,r.depth)} fill={colors[r.kind]} fillOpacity={reference?'.38':'.7'} stroke={r.id===selected?'#be8226':'#7c8c82'} strokeWidth={r.id===selected?3:1} vectorEffect="non-scaling-stroke"/><text x={r.x+r.width/2} y={r.z+r.depth/2} textAnchor="middle" fontSize={Math.min(view.width/65,r.width/9)} fill="#34483c" fontWeight="800">{r.name}</text><text x={r.x+r.width/2} y={r.z+r.depth/2+Math.min(view.width/60,r.width/8)} textAnchor="middle" fontSize={Math.min(view.width/85,r.width/12)} fill="#455d50">{(r.width/factor).toFixed(2)} × {(r.depth/factor).toFixed(2)} {unit}</text></g>)}
      ...
      {notice&&!error&&<div className="bp-note">{notice}</div>}
    </section></div>
  </dialog>;
}
