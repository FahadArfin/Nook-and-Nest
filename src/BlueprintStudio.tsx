...
export function BlueprintStudio({onClose,onCreated}:{onClose:()=>void;onCreated?:()=>void}) {
  ...
  const [gesture,setGesture]=useState<{a:Point;b:Point;before:BlueprintDraft;selected?:string;handle?:string;mode:Mode;view:typeof view}>();
  ...
  async function load(nextFile:File,nextPage=1,nextRotation=0,clear=false) {
    ...
    try {
      ...
      let costStatus='';
      const detection=await recognizeReference(result,controller.signal,{model:'gpt-5.6-luna',status:text=>{costStatus=text;}});if(generation!==loadGeneration.current)return;
  ...
  function down(e:ReactPointerEvent<SVGSVGElement>) {
    ...
    if(handle&&resizeId){const p=point(e);setGesture({a:p,b:p,before:draft,selected:resizeId,handle,mode:'resize',view});return;}
    ...
    if(mode==='combine'){const g=groupedRooms.find(r=>r.parts.some(p=>p.id===hit));if(g)setCombineIds(ids=>ids.includes(g.id)?ids.filter(id=>id!==g.id):[...ids,g.id]);return;}
  ...
  function combineSelected() {
    try{let next=draft;const remaining=[...combineIds],first=remaining.shift();if(!first)return;
      while(remaining.length){let joined=false;for(let i=0;i<remaining.length;i++){try{next=combineBlueprintRooms(next,first,remaining[i],base.gridSizeMm);remaining.splice(i,1);joined=true;break;}catch{/* Try another adjacent piece in the selection. */}}if(!joined)throw new Error('Move the selected rooms together so they share edges.');}
      commit(next);setSelected(first);setCombineIds([]);setMode('select');
  ...
  return <dialog ref={dialogRef} className="blueprint-dialog" data-theme={(appearance?.dark??savedDark)?'dark':'light'} aria-labelledby="blueprint-title" onCancel={e=>{e.preventDefault();if(menu||palette||mode!=='select'){setMenu(undefined);setPalette(undefined);setMode('select');setCombineIds([]);}else close();}} onKeyDown={e=>{e.stopPropagation();if((e.key==='Delete'||e.key==='Backspace')&&!(e.target as Element).closest('input,select,textarea')&&!review){e.preventDefault();remove();}}}>
    <header className="bp-header"><div><span className="eyebrow">{floorName} · unsaved draft</span><h1 id="blueprint-title">Floor plan studio</h1></div><nav className="bp-menus" aria-label="Studio menus"><button disabled={loading||review} aria-expanded={menu==='file'} onClick={()=>setMenu(menu==='file'?undefined:'file')}>File</button><button aria-expanded={menu==='view'} onClick={()=>setMenu(menu==='view'?undefined:'view')}>View</button></nav><button onClick={close} aria-label="Close floor plan studio"><X size={22}/></button></header>
    <div className="bp-file-menu" hidden={menu!=='file'}><button disabled={loading} onClick={()=>{uploadRef.current?.click();setMenu(undefined);}}><FileArrowUp/> Import PDF or image…</button><small>New scans use online analysis and may incur an API charge. Cached scans are reused.</small><input hidden ref={uploadRef} type="file" accept=".pdf,.png,.jpg,.jpeg,.webp" aria-label="Upload floor plan reference" onChange={e=>{const f=e.target.files?.[0];e.target.value='';if(!f)return;if(draft.rooms.length&&!window.confirm('Analyze a new floor plan from this file? This replaces only the unconfirmed drawing; your 3D home is unchanged.'))return;void load(f,1,0,true);}}/>
...
<button onClick={()=>setNotice(clearRecognitionCache()?'Saved analyses cleared. Future imports may incur a charge.':'Could not clear cache.')}>Clear saved analyses</button></div>
    ...
    {loading&&<div className="bp-analysis-status" role="status"><strong>Reading your floor plan…</strong><p>Finding rooms and printed dimensions. Detailed scans can take a few minutes.</p><button onClick={()=>{analysisAbort.current?.abort();loadGeneration.current++;setLoading(false);setNotice('Analysis canceled. Your drawing is unchanged.');}}>Cancel analysis</button></div>}<div className={`bp-workspace ${sidebar||review?'':'bp-sidebar-hidden'}`} style={pendingScale?{display:'none'}:undefined} inert={loading||!!pendingScale} aria-busy={loading}><aside className="bp-sidebar" hidden={!sidebar&&!review}>
      {!review&&<>
        <section><h2>Rooms & spaces · {spaces.length}</h2><div className="bp-room-list">{spaces.map(r=><div key={r.id} className="bp-room-row">{mode==='combine'&&<input type="checkbox" aria-label={`Combine ${r.name}`} checked={combineIds.includes(r.id)} onChange={e=>setCombineIds(ids=>e.target.checked?[...ids,r.id]:ids.filter(id=>id!==r.id))}/>}<button className={r.id===room?.id?'active':''} onClick={()=>{setSelected(r.id);if(mode!=='combine')setMode('select');}}><span style={{background:colors[r.kind]}}/>{r.name}<small>{(r.width/factor).toFixed(2)} × {(r.depth/factor).toFixed(2)} {unit}</small></button></div>)}</div>{mode==='combine'&&<button className="primary" disabled={combineIds.length<2} onClick={combineSelected}>Combine selected ({combineIds.length})</button>}</section>
        ...
        {room&&room.parts.length>1&&selectedPart&&<section className="bp-selection"><h2>Rectangles in {room.name}</h2>{room.parts.map((part,index)=><button key={part.id} className={selected===part.id?'active':''} onClick={()=>setSelected(part.id)}>Rectangle {index+1}</button>)}<button onClick={()=>{commit(separateBlueprintRectangle(draft,selectedPart.id));}}>Separate rectangle</button><button onClick={()=>{commit({...draft,rooms:draft.rooms.filter(r=>r.id!==selectedPart.id)});setSelected(room.parts.find(r=>r.id!==selectedPart.id)?.id);}}>Delete rectangle</button></section>}
      ...
      <div className="bp-instruction" role="status">{snapped?'Shared wall snapped':loading?'Reading rooms and dimensions…':review?'Review your floor plan before creating the 3D home.':mode==='room'?'Drag from one room corner to the opposite corner.':mode==='cut'?'Drag along an inside wall to remove only that section.':mode==='combine'?'Select rooms on the plan or in the list, then Combine selected.':mode==='wall'?'Drag a straight inside wall; gold points show snapped ends.':mode==='fixture'?(entrance?'Click a wall to add an entrance without a door.':'Click to place the selected fixture, then drag or edit its dimensions.'):mode==='scale'?'Drag along a known measurement on the reference.':mode==='pan'?'Drag to move around the drawing.':'Drag to move · corners to resize'}</div>
      <svg ref={svgRef} className={`bp-canvas mode-${mode}`} role="img" aria-label="Top-down floor plan drawing" viewBox={`${view.x} ${view.z} ${view.width} ${view.height}`} onPointerDown={e=>{setMenu(undefined);down(e);}} onPointerMove={move} onPointerUp={up} onPointerCancel={()=>{if(gesture)setDraft(gesture.before);setGesture(undefined);setSnapped(false);}}>
        ...
        {room&&!review&&mode==='select'&&room.parts.map((part,index)=>[['nw',part.x,part.z],['ne',part.x+part.width,part.z],['sw',part.x,part.z+part.depth],['se',part.x+part.width,part.z+part.depth]].map(([handle,x,z])=><rect key={`${part.id}-${handle}`} data-handle={handle} data-part={part.id} aria-label={`Resize ${room.name} rectangle ${index+1} ${handle}`} x={Number(x)-handleSize/2} y={Number(z)-handleSize/2} width={handleSize} height={handleSize} fill="#f5c963" stroke="#795118" strokeWidth="2" vectorEffect="non-scaling-stroke" style={{cursor:handle==='nw'||handle==='se'?'nwse-resize':'nesw-resize'}}/>))}
      ...
      <button aria-label="Combine rooms" className={mode==='combine'?'active':''} disabled={groupedRooms.length<2} onClick={()=>{setMode('combine');setSidebar(true);setCombineIds(room?[room.id]:[]);}}><SquaresFour size={24}/><span>Combine</span></button>
