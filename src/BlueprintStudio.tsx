...
export function BlueprintStudio({onClose,onCreated}:{onClose:()=>void;onCreated?:()=>void}) {
  ...
  async function load(nextFile:File,nextPage=1,nextRotation=0,clear=false) {
    ...
    try {
      ...
      setNotice(`Detected ${recognized.draft.rooms.length} room areas and ${recognized.draft.fixtures.length} fixtures / openings. Scale comes from the printed dimensions. Review names and geometry, then create your 3D home.`);
  ...
  return <dialog ref={dialogRef} className="blueprint-dialog" aria-labelledby="blueprint-title" onCancel={e=>{e.preventDefault();close();}} onKeyDown={e=>e.stopPropagation()}>
    ...
    {loading&&<button onClick={()=>{analysisAbort.current?.abort();loadGeneration.current++;setLoading(false);setNotice('Analysis canceled. Your drawing is unchanged.');}}>Cancel analysis</button>}<div className="bp-workspace" inert={loading} aria-busy={loading}><aside className="bp-sidebar">
      {!review&&<>
