...
import {BacksplashShortcut} from './KitchenDetails';
...
export function ToolBrowser({onPlace}:{onPlace(item:CatalogItem):void}){
 const s=usePlanner();
 ...
 useEffect(()=>{
  ...
  else if(['wall','wall-cut','measured-room','paint','erase'].includes(s.tool)){setDestination('Build');setBuild(s.tool==='wall'||s.tool==='wall-cut'?'Walls':'Floor area')}
 ...
 return <aside className="inspector-panel tool-browser" aria-label="Home tools">
  ...
  {destination==='Build'&&<><div className="task-subtabs" aria-label="Building tools">{['Walls','Openings','Floor area'].map(t=><button key={t} aria-pressed={build===t} onClick={()=>{s.setTool('select');setBuild(t)}}>{t}</button>)}</div><div className="task-content"><h2>{build==='Walls'?'Shape your walls':build==='Openings'?'Doors & windows':'Shape your floor'}</h2>{build==='Walls'?<WallConstructionControls embedded/>:build==='Openings'?<><div className="build-choices">{([{name:'Doors',icon:Door},{name:'Windows',icon:FrameCorners},{name:'Stairs',icon:GridFour}] as const).map(({name,icon:Icon})=><button key={name} onClick={()=>{s.setCategory(name);s.setSearch('');s.setTool('select')}}><Icon size={32}/><strong>{name}</strong><small>Browse the library</small></button>)}</div><BacksplashShortcut/></>:<><div className="task-subtabs"><button aria-pressed={s.tool==='paint'} onClick={()=>s.setTool('paint')}><GridFour/> Add floor</button><button aria-pressed={s.tool==='erase'} onClick={()=>s.setTool('erase')}><Trash/> Erase floor</button></div><p className="tool-hint">Drag a rectangle, then confirm.</p><MeasuredRoom/></>}</div></>}
  {destination==='Landscape'&&<><div className="task-subtabs" aria-label="Landscape tools">{['Terrain','Plants','Surroundings'].map(t=><button key={t} aria-pressed={landscape===t} onClick={()=>{s.setTool('select');setLandscape(t)}}>{t}</button>)}</div>{landscape==='Plants'?<PlantingSettings onPlace={onPlace}/>:<div className="task-content">{landscape==='Terrain'?<TerrainSettings/>:<Surroundings/>}</div>}</>}
 </aside>;
}
...
function SurfaceBrowser(){
 ...
 return <><div className="task-subtabs" aria-label="Surface">{(['Floor','Walls'] as const).map(t=><button key={t} aria-pressed={target===t} onClick={()=>{s.setTool('select');setTarget(t);setFamily('All');setSearch('');setPending(undefined)}}>{t}</button>)}</div><div className="task-content"><h2>{target==='Floor'?'Finish your floor':'Color your walls'}</h2><label className="scope-control">Apply to<select aria-label="Finish area" value={scope} onChange={e=>{setScope(e.target.value);setPending(undefined);s.setTool('select')}}><option value="sections">{target==='Floor'?'Paint a section':'Selected wall'}</option><option value="whole">{target==='Floor'?'Whole floor':'All walls on this floor'}</option></select></label><div className="tool-search"><MagnifyingGlass/><input aria-label="Search finishes" placeholder="Search materials…" value={search} onChange={e=>setSearch(e.target.value)}/></div><div className="material-types" aria-label="Material type">{['All',...new Set(finishes.map(f=>f.family))].map(f=><button key={f} aria-pressed={family===f} onClick={()=>setFamily(f)}>{f}</button>)}</div><div className="tool-visual-grid">{visible.map(f=><button key={f.id} aria-label={`${target}: ${f.name}`} aria-pressed={(pending??current)===f.id} onClick={()=>{if(scope==='whole')setPending(f.id);else if(target==='Walls'&&s.selectedWallId)s.finishWall(s.selectedWallId,f.id);else s.setSurfaceBrush(target==='Floor'?'floor-finish':'wall-finish',f.id)}}><img src={f.texture} alt=""/><strong>{f.name}</strong>{(pending??current)===f.id&&<Check className="choice-check" weight="bold"/>}</button>)}</div>{!visible.length&&<p role="status">No matching materials.</p>}</div><footer className="tool-footer">{pending?<button className="primary" onClick={()=>{s.setFloorFinish(kind,pending);setPending(undefined)}}>Apply to {target==='Floor'?'whole floor':'all walls'}</button>:<p>{scope==='whole'?'Choose a material to preview your choice.':target==='Floor'?'Choose a finish. Drag a section, then confirm.':'Select a wall, then choose its finish.'}</p>}{(s.tool==='floor-finish'||s.tool==='wall-finish')&&<button onClick={()=>s.setTool('select')}>Done editing</button>}</footer></>;
}

function Surroundings(){
 const s=usePlanner(),e=s.plan.environment??{background:'plain',grass:'off'};
 return <><h2>Your surroundings</h2><label>Backdrop<select aria-label="Surroundings" value={e.background} onChange={event=>s.setEnvironment({background:event.target.value as typeof e.background})}>{sceneryOptions.map(v=><option key={v} value={v}>{({plain:'Plain ground',city:'City neighborhood',suburban:'Suburban homes',rural:'Rural hills',farm:'Farm fields',medieval:'Fantasy medieval'})[v]}</option>)}</select></label><label>Ground grass<select aria-label="Ground grass" value={e.grass} onChange={event=>s.setEnvironment({grass:event.target.value as typeof e.grass})}><option value="off">Off</option><option value="sparse">Light grass</option><option value="lush">Fuller grass</option></select></label><p className="tool-hint">Outside your home only.</p><button onClick={()=>{s.setCategory('Outdoor');s.setSearch('');s.setTool('select')}}>Browse outdoor furniture</button></>;
}
