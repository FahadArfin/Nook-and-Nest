...
export function modelAssetPath(id:string,preview=false){
  ...
  return `/models/furniture/${id}.glb?v=${modern?'batch12-modern-1':id==='backdrop-city'?'toronto-massing-aerial-1':id.startsWith('backdrop-')?'scenery-toronto-3':'lossless-mesh-1'}`;
}
