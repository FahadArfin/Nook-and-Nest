...
const definitions:Array<Omit<AgentTool,'execute'>&{run:(input:any)=>unknown}>=[
  ...
  {name:'nook_set_view',title:'Inspect a floor or view',description:'Switch the visible floor and/or top/isometric/dollhouse view and select an existing piece for human inspection. No furnishing changes. View mode participates in normal undo/local autosave. A newly selected piece gently focuses once; manual camera input cancels that transition. Finish or discard any proposal first.',inputSchema:object({floorId:text(),mode:choice('top','isometric','dollhouse'),selectedId:text()}),annotations:{readOnlyHint:false,untrustedContentHint:false},run:(input)=>{
