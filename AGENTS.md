...
Production floor-plan promotion (September 12, user-approved): merge the reviewed wall-first workflow and Pan/zoom footprint lock into master and publish to the original public Nook & Nest Site. Preserve production hosting identity and master-only release artifacts. Local extraction remains the default; Luna is explicit experimental import. Earlier Beta-only restrictions do not block this approved promotion.

Floor-planner toolkit (September 24): use Spaceplanner as a functional reference for the floor-plan studio only. Group drawing, openings, measurements and annotation tools in a desktop left rail with a right properties/layers panel and reachable mobile drawers. Preserve default Pan, imported-footprint locking, local wall-first import, explicit draft saving and reviewed 3D conversion. Concave rooms must retain their real floor area; dimensions and notes are drawing annotations, not physical walls.

Custom-room drawing preference (September 24): make nearby walls and drafted corners easy to snap to at every zoom level. Show the proposed connection before clicking and a clear starting-corner target for closing the room. Preserve right angles, exact shared boundaries, Snap off and one-step room Undo.

Automatic custom-room drafting (September 25, revised): the user rejected the separate Connected rooms preview/apply workflow. Custom room must create and name each enclosed space immediately, including closures against existing room edges and crossing partitions. Keep drawing available after creation, remove temporary tails, support Undo, and show alignment guides from existing room corners as well as the active outline. Imported wall-first plans retain Generate regions and footprint locking.

Floor-plan usability (September 25): consolidate room creation under Draw room; ask for exact measurements before creating a measured room. Properties follow the active task and must not show the previous room's edit form while drawing. Keep both dimensions visible, drawing feedback stable, and review actionable with actual replacement impact. Recover drafts automatically on this device without changing the 3D home or implicitly saving online. Preserve automatic room creation and Pan / zoom by default.

Custom room correction (September 26): Keep Custom room directly visible as the primary drawing tool, including the empty-plan start action. The user rejected the Draw room menu and large yellow square selection/resize controls. Use thin gold wall lines, circular connection cues and automatic named room creation on closure. Support actual diagonal and irregular polygons through saving and 3D conversion; do not force right angles or fill their bounding rectangles. Keep Pan / zoom as the navigation default and imported footprints locked. This supersedes the earlier Draw room consolidation and right-angle-only drafting directions.

Custom-room alignment (September 26): keep dashed horizontal/vertical construction guides from prior corners and existing rooms, soft endpoint alignment snapping, equal-wall-length tick cues and 90-degree feedback while drawing. Make it clear where to stop a returning wall without restoring yellow boxes or restricting irregular/angled rooms. Snap off must disable all magnetic constraints.
