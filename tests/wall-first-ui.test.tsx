...
it('pans after import and locks selected footprint geometry until explicitly edited',async()=>{
 ...
 fireEvent.click(screen.getByRole('button',{name:'Select and move rooms'}));fireEvent.pointerDown(rect,{button:0,clientX:100,clientY:100});fireEvent.pointerMove(canvas,{clientX:500,clientY:500});fireEvent.pointerUp(canvas);
 ...
 fireEvent.click(screen.getByRole('button',{name:'2. Edit footprint'}));expect(canvas.querySelector('[data-handle]')).toBeNull();expect(screen.getByRole('button',{name:'Move room left'})).toBeEnabled();
