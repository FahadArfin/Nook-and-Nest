...
import {shareImages,shouldCompressModel} from '../scripts/optimize-model-assets.mjs';
...
test('every large model and detailed appliance decompresses to identical geometry, metadata, materials and animation',async()=>{
 ...
 for(const file of readdirSync('public/models/furniture').filter(f=>f.endsWith('.glb'))){
  const input=shareImages(readFileSync('public/models/furniture/'+file),()=>{});if(!shouldCompressModel(file,input.length))continue;
...
test('Babylon imports the compressed appliances, aquariums and largest tree using the shipped decoder with identical mesh data',async()=>{
 ...
 try{
  for(const id of ['reef-aquarium','planted-aquarium','desktop-aquarium','weeping-willow',...JSON.parse(readFileSync('src/applianceDetailIds.json','utf8'))]){
