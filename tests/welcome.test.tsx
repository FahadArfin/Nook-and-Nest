...
it('navigates from 3D to Studio and back/forward with native browser history',async()=>{
 ...
 render(<Welcome Editor={Navigable}/>);const start=screen.getByRole('button',{name:/Design in 3D/});await waitFor(()=>expect(start.hasAttribute('disabled')).toBe(false));fireEvent.click(start);fireEvent.click(screen.getByRole('button',{name:'Floor plan'}));await screen.findByRole('dialog',{name:'Floor plan studio'});const plan=usePlanner.getState().plan;window.history.back();await waitFor(()=>expect(screen.queryByRole('dialog',{name:'Floor plan studio'})).toBeNull());expect(screen.getByRole('region',{name:'Editor'})).toBeTruthy();expect(usePlanner.getState().plan).toBe(plan);window.history.forward();const studio=await screen.findByRole('dialog',{name:'Floor plan studio'});fireEvent.click(within(within(studio).getByRole('toolbar',{name:'Floor plan editing'})).getByRole('button',{name:'Add room by dimensions'}));const confirm=vi.spyOn(window,'confirm').mockReturnValue(false);window.history.back();await waitFor(()=>expect(confirm).toHaveBeenCalled());await waitFor(()=>expect(window.history.state.nookNavigation.screen).toBe('studio-editor'));expect(screen.getByRole('dialog',{name:'Floor plan studio'})).toBeTruthy();confirm.mockReturnValue(true);window.history.back();await waitFor(()=>expect(screen.queryByRole('dialog',{name:'Floor plan studio'})).toBeNull());confirm.mockRestore();
});
