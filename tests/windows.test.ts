...
describe("wall-mounted window planning",()=>{
  ...
  it("round-trips all six styles exactly through backup and sharing",()=>{
    ...
    expect(p.furniture).toHaveLength(6);expect(parsePlan(serializePlan(p)).furniture).toEqual(p.furniture);expect(decodeShare(encodeShare(p)).furniture).toEqual(p.furniture);
  });
