...
describe('garden planting brush',()=>{
 it('turns a Babylon pointer stroke into a live preview then one committed stroke',()=>{
  ...
  try{controller.initializeControllers();controller.bindPointers();const fire=(type:number)=>scene.onPointerObservable.notifyObservers({type,event:{button:0,pointerId:1},pickInfo:null} as any);
