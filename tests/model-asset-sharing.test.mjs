...
test('shares images for the entire catalog while preserving every geometry byte, material and moving node',()=>{
 let savings=0;const allImages=new Map();
 for(const file of readdirSync('public/models/furniture').filter(f=>f.endsWith('.glb'))){
  const input=readFileSync('public/models/furniture/'+file),output=shareImages(input,(name,data)=>allImages.set(name,data));assert.equal(output.readUInt32LE(8),output.length);const source=parse(input),result=parse(output);assert.deepEqual(result.g.nodes,source.g.nodes);assert.deepEqual(result.g.meshes,source.g.meshes);assert.deepEqual(result.g.materials,source.g.materials);
 ...
 savings-=[...allImages.values()].reduce((sum,b)=>sum+b.length,0);assert.ok(savings>60_000_000);assert.equal(allImages.size,12);
});
