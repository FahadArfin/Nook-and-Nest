...
it('does not schedule a focus motion on top-view selection',()=>{
 ...
 Object.assign(c,{activePlan:p,selectedId:'a',selectedNode:{position:Vector3.Zero()},editingKey:'',rotationMode:false,camera:{radius:20}});c.updateEditingGuides();expect(c.focusMotion).toBeUndefined();expect(c.camera.radius).toBe(20);
});
...
it('restores the exact editor camera after presenting a different view',()=>{
 ...
 Object.assign(c,{camera,engine:{resize:vi.fn(),getRenderWidth:()=>800,getRenderHeight:()=>600},cancelFocus:vi.fn()});vi.stubGlobal('requestAnimationFrame',vi.fn());c.beginHomePreview();c.showHomeShot({name:'room',x:9,y:1,z:9,radius:5,alpha:3,beta:.6},true);expect(camera.radius).toBe(5);c.endHomePreview();expect(camera.target.asArray()).toEqual([1,2,3]);expect(camera).toMatchObject({alpha:1,beta:2,radius:19,mode:1});
});
