...
describe('construction and furnishing improvements',()=>{
 ...
 it('keeps solarium glazing close to floor and ceiling',()=>{const p=setup();p.floors[0].heightMm=2450;const w=snapWindow(p,piece(p,'window-solarium'));expect(w.elevationMm).toBe(0);expect(windowProblem(p,w)).toBeUndefined()});
